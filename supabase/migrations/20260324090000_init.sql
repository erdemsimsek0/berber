...
-- Prevent overlapping double booking on same staff except cancelled/no_show
...
create policy "business scoped staff_services" on public.staff_services for all using (
  exists (
    select 1
    from public.staff s
    where s.id = staff_services.staff_id
      and s.business_id in (select * from public.current_user_business_ids())
  )
) with check (
  exists (
    select 1
